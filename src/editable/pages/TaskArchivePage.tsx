...
const taskGrid: Record<TaskKey, string> = {
  ...
  profile: 'grid gap-4 sm:grid-cols-2 lg:grid-cols-3',
}

// Shared premium surface: hairline border, soft radius, smooth lift on hover.
const cardBase = 'group block rounded-md border border-[var(--editable-border)] bg-white transition duration-300 hover:border-[var(--slot4-accent)] hover:shadow-[0_12px_30px_rgba(15,23,42,0.08)]'
    ...
    <EditableSiteShell>
      <main style={taskThemeStyle(task)} className="min-h-screen bg-[var(--tk-bg)] text-[var(--tk-text)]">
        <div className="mx-auto max-w-6xl px-4 py-6">
          <Ads slot={task === 'profile' ? 'in-feed' : 'header'} showLabel eager className="mx-auto w-full" />
        </div>
        <header className="relative overflow-hidden border-y border-[var(--editable-border)] bg-[var(--slot4-warm)]">
          <div className="relative mx-auto max-w-[var(--editable-container)] px-4 py-8 sm:px-6 lg:px-6">
            ...
            <h1 className="editable-display mt-4 max-w-3xl text-balance text-[2.25rem] font-black leading-tight sm:text-4xl">
            ...
            {voice?.chips?.length ? (
              <div className="mt-8 flex flex-wrap gap-2.5">
                {voice.chips.map((chip) => (
                  <span key={chip} className="rounded-md border border-[var(--editable-border)] bg-white px-3.5 py-1.5 text-xs font-medium text-[var(--tk-muted)]">{chip}</span>
                ))}
              </div>
            ) : null}

            <div className="mt-8 flex flex-col gap-4 border-t border-[var(--editable-border)] pt-5 sm:flex-row sm:items-center sm:justify-between">
              ...
              <form action={basePath} className="flex items-center gap-2.5">
                <div className="relative">
                  <select
                    ...
                    className="h-11 appearance-none rounded-md border border-[var(--editable-border)] bg-white pl-4 pr-10 text-sm font-medium text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
                ...
                <button className="inline-flex h-11 items-center rounded-md bg-[var(--tk-accent)] px-5 text-sm font-semibold text-[var(--tk-on-accent)] transition hover:opacity-90">Apply</button>
              </form>
            </div>
          </div>
        </header>
